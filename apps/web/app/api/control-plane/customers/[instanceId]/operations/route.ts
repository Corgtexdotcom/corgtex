...
import { z } from "zod";
import { getControlPlaneCustomer, recordBreakGlassSupportNote, runCustomerSupportOperation } from "@corgtex/domain";
...
export async function GET(
  ...
  try {
    ...
    const customer = await getControlPlaneCustomer(actor, instanceId);
    return NextResponse.json({ operations: customer.supportOperations });
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    const operation = raw?.action === "support.break_glass_note"
      ? await recordBreakGlassSupportNote(actor, { instanceId, ...breakGlassSchema.parse(raw) })
      : await runCustomerSupportOperation(actor, { instanceId, ...operationSchema.parse(raw) });
