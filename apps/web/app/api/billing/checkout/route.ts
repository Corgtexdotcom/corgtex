import { createStripeCheckoutSession } from "@corgtex/domain";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const session = await createStripeCheckoutSession(actor, workspaceId);
    if (!session.url) {
      return NextResponse.json({ error: "Stripe checkout session did not include a URL." }, { status: 502 });
    }
