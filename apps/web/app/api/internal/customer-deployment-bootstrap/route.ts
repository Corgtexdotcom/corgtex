...
function configuredBootstrapTokenHash() {
  const configuredHash = process.env.CORGTEX_CUSTOMER_DEPLOYMENT_BOOTSTRAP_TOKEN_HASH?.trim();
  if (configuredHash && /^[a-f0-9]{64}$/i.test(configuredHash)) {
    return configuredHash.toLowerCase();
  }

  const rawToken = process.env.CORGTEX_CUSTOMER_DEPLOYMENT_BOOTSTRAP_TOKEN?.trim();
  return rawToken ? sha256Hex(rawToken) : null;
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const existingAppliedRun = await prisma.customerDeploymentBootstrapRun.findFirst({
      where: {
        customerSlug: body.customerSlug,
        status: "applied",
      },
    ...
    if (existingAppliedRun) {
      throw new AppError(409, "BOOTSTRAP_ALREADY_APPLIED", "Customer deployment bootstrap has already been applied.");
    }

    const existingRun = await prisma.customerDeploymentBootstrapRun.findUnique({
      where: {
    ...
    const run = existingRun
      ? await prisma.customerDeploymentBootstrapRun.update({
      ...
      : await prisma.customerDeploymentBootstrapRun.create({
        data: {
          customerSlug: body.customerSlug,
          bundleUri: body.bundleUri,
          bundleChecksum: body.checksum.toLowerCase(),
    ...
    const consumed = await prisma.customerDeploymentBootstrapRun.updateMany({
      where: {
        id: run.id,
        bootstrapTokenHash: tokenHash,
