  ...
  return (
    <section className="stack" style={{ gap: 20 }} data-finance-surface="practice-dashboard">
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <div style={METRIC}><div style={LABEL}>Active projects</div><div style={{ fontSize: 26, marginTop: 6 }}>{summary.activeProjects}</div></div>
        <div style={METRIC}><div style={LABEL}>Budget</div><div style={{ fontSize: 26, marginTop: 6 }}>{usd(summary.budgetCents)}</div></div>
        <div style={METRIC}><div style={LABEL}>Used</div><div style={{ fontSize: 26, marginTop: 6 }}>{usd(summary.usedCents)}</div></div>
        <div style={METRIC}><div style={LABEL}>Remaining</div><div style={{ fontSize: 26, marginTop: 6 }}>{usd(summary.remainingCents)}</div></div>
        <div style={METRIC}><div style={LABEL}>Margin</div><div style={{ fontSize: 26, marginTop: 6 }}>{marginPct(summary.marginBps)}</div></div>
      </div>
            ...
            <tbody>
              {attention.map((item, idx) => (
                <tr key={`${item.projectId}-${item.issue}-${idx}`} style={{ borderTop: "1px solid var(--line)" }}>
                  ...
                  <td style={{ padding: "8px 16px", color: item.issue === "setup" ? "var(--muted)" : "var(--danger)" }}>{item.issue}</td>
            ...
            <tbody>
              {projects.map((p) => (
                <tr key={p.id} style={{ borderTop: "1px solid var(--line)" }}>
                  ...
                  <td style={{ padding: "8px 16px", textAlign: "right" }}>{weeksLabel(projectBudgetRunwayWeeks(p))}</td>
                </tr>
              ))}
            </tbody>
          </table>
