...
describe("support request allowlist", () => {
  ...
  it.each(["support-setup", "support-configuration"])("allows only exact sanitized %s with a current grant", async endpoint => {
    await expect(requireSupportRequestAccess(actor, `/api/workspaces/ws-1/${endpoint}`)).resolves.toBeUndefined();
    expect(membership).not.toHaveBeenCalled();
    await expect(requireSupportRequestAccess(actor, `/api/workspaces/ws-1/${endpoint}/export`)).rejects.toThrow();
    grant.mockResolvedValue({ isActive: false });
    await expect(requireSupportRequestAccess(actor, `/api/workspaces/ws-1/${endpoint}`)).rejects.toMatchObject({ code: "SUPPORT_ACCESS_REQUIRED" });
  });
