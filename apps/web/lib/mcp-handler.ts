...
function protectedResourceMetadataUrl(request: NextRequest, workspaceId?: string) {
  if (workspaceId) return getWorkspaceMcpMetadataUrl(workspaceId);
...
function mcpAuthErrorResponse(request: NextRequest, error: AppError, challengeScopes: string[], workspaceId?: string) {
    ...
    { error: { code: error.code, message: error.message } },
    {
      ...
      headers: {
        "WWW-Authenticate": bearerChallenge({
          ...
          resourceMetadataUrl: protectedResourceMetadataUrl(request, workspaceId),
 ...
 */
export async function POST(request: NextRequest, workspaceId?: string) {
  beginAuthorizationContext();
  let server;
  try {
    if (workspaceId) assertCanonicalMcpEndpoint(request, workspaceId);
    const authHeader = request.headers.get("authorization");
    const sessionCtx = await authenticateMcpRequest(authHeader, {
      resourceUrl: workspaceId ? getWorkspaceMcpResource(workspaceId) : getPublicRequestUrl(request),
      workspaceId,
    });
    const body = await request.json();
    assertBoundWorkspace(request, body, sessionCtx.workspaceId);
    ...
    const response = await withMcpConnectionExecution({ workspaceId: sessionCtx.workspaceId, mcpOrigin: sessionCtx.mcpOrigin },
    ...
    if (error instanceof Error && error.message === "MCP_AUTHORIZATION_REVOKED") {
      return mcpAuthErrorResponse(request, new AppError(401, "UNAUTHENTICATED", "MCP connection authorization is unavailable."), MCP_CONNECTOR_DEFAULT_SCOPES, workspaceId);
    }
    if (error instanceof McpInsufficientScopeError) {
      return mcpAuthErrorResponse(request, error, [...error.grantedScopes, error.requiredScope], workspaceId);
    }
    if (error instanceof AppError && error.status === 401) {
      return mcpAuthErrorResponse(request, error, MCP_CONNECTOR_DEFAULT_SCOPES, workspaceId);
    }
...
export function assertBoundWorkspace(request: Request, body: unknown, workspaceId: string) {
  for (const header of ["x-workspace-id", "x-corgtex-workspace-id"]) {
    const value = request.headers.get(header);
    if (value && value !== workspaceId) throw new AppError(403, "MCP_WORKSPACE_SCOPE_REQUIRED", "Connection workspace mismatch.");
  }
  const pending = [body];
  while (pending.length) {
    ...
    for (const [key, item] of Object.entries(value)) {
      if (["workspaceId", "workspace_id"].includes(key) && item !== workspaceId) {
