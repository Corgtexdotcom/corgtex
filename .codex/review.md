...
11. **CI red** — any required check failed: `Lint, Typecheck & Test`, `Database Sync`, `Build`, `Docs Validation`, `Plan Present`, `Scope Check`, `Secret Scan`, `Diff Size`, `Client Data Scan`, or `Review Snapshot Integrity`. Before the first attested approval, `Review Snapshot Integrity` is expected to be red; follow the approval sequence below, rerun it immediately after approving, and do not treat the review as complete until the rerun is green.
...
16. **Mutated or unattested snapshot** — the latest live `beepto-codex` approval lacks the exact `rsi/v1` attestation below, or its head, base, body, labels, review commit, or live policy verdict no longer matches.
...
Immediately before approving, work from a clean checkout of current `origin/main`, fetch the PR live with `gh api repos/{owner}/{repo}/pulls/{number}`, and pass that JSON object directly to `computeSnapshot()` and `buildAttestationPayload()` exported by `scripts/review-snapshot-integrity.mjs`. Do not read these fields from a checkout or webhook. Confirm the resulting single-line payload exactly matches the payload in the latest `Review Snapshot Integrity` job summary, then put it in exactly one review-body block:

````text
All review criteria pass. Approved by beepto-codex.

```review-snapshot-attestation
{"v":"rsi/v1","pr":<number>,"headSha":"<40-hex>","baseSha":"<40-hex>","bodyDigest":"<64-hex>","labelDigest":"<64-hex>"}
```
````

The shared functions hash the exact UTF-8 body bytes and encode labels as a four-byte big-endian count followed by byte-sorted, four-byte-length-prefixed UTF-8 names, preserving case and duplicates. Verify `gh api user --jq .login` is exactly `beepto-codex`, submit the approval, then rerun the latest `pull_request_target` run of `review-snapshot-integrity.yml` with `gh run rerun <run-id>`. Re-read the PR and check result; if the snapshot changed or the rerun is not green, dismiss the approval and request changes. The workflow intentionally does not use `pull_request_review`, because that event evaluates workflow code from the PR merge ref and is not a safe write-token trust boundary.

Submit the prepared body with:
```
gh pr review <number> --approve --body-file <attestation-body-file>
```
...
- **`force-merge` label:** request changes. This label is a hard failure of `Review Snapshot Integrity`; no agent may apply it, act on it, or bypass branch protection. Any emergency override is performed directly by a human administrator outside the agent pipeline.
