...
10. **Frontend control drift** — a frontend diff introduces mismatched sizing, typography, spacing, or alignment for controls in the same family, or adds inline `fontSize`, `padding`, `width`, `minWidth`, or `alignSelf` overrides on buttons, filters, checkbox controls, or table row actions instead of using a named design-system class or shared primitive.
