...
- Every initial (first) planning pass must be authored only by **Claude
  ...
  initial Planner.
- Broad analysis is permitted for Kimi, Gemini, Claude Sonnet, Codex
  Terra, and other models, but analysis output is advisory evidence
  only: it must never be copied, promoted, or treated as the initial
  plan contract. An allowed Planner (Opus or Sol) independently
  evaluates any prior analysis before authoring the plan.
- Execution permission is narrower than analysis permission:
  only Kimi or Gemini may act as Executor. Claude Opus, Codex Sol,
  Claude Sonnet, Codex Terra, and every other model must not implement,
