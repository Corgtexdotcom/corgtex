...
- [ ] A URL resolution helper is added to map `entityType` + `entityId` to workspace routes.
- [ ] Notification items in the dashboard wrap their title in a `<Link>` pointing to the correct entity type's list.
- [ ] Notifications without a resolvable entity type fall back to plain text.
- [ ] Approval pending items wrap their `subjectType` string in a `<Link>` pointing to the relevant entity list.
- [ ] The "Approve" button remains a separate action and is unaffected by the link.
