...
2. Run `AGENT_API_KEY="..." npm run prisma:seed` to create the agent user (ADMIN role in the default org).
3. Start the server (`npm run dev`).
4. Call `/api/...` with `Authorization: Bearer agent-<AGENT_API_KEY>`.

<Warning>
  The agent user has `ADMIN` role on the default organization. Wallet-dependent flows (e.g. optimistic finance) require additional setup.
</Warning>
