...
**role separation** (Planner, Executor, and Reviewer are independent
agents, none of which can paper over another's mistakes) plus
**mechanical CI gates** (Codex cannot merge a PR whose required checks
are red).
...
| --- | --- | --- | --- | --- |
| **Planner** | Claude Opus (Codex Sol fallback) | No | Draft only | No |
| **Executor** | Gemini in Antigravity, or Kimi | Yes | Yes (ready) | No |
...
- Try Opus first. Use Sol only when Opus is unavailable,
  unauthenticated, or out of subscription quota.
- If neither is available, stop and request human direction. Do not
  substitute Kimi, Gemini, Claude Sonnet, Codex Terra, or any other
  model as initial Planner.
...
The Planner (Claude Opus, or Codex Sol as fallback):
...
The Executor (Gemini in Antigravity, or Kimi):
