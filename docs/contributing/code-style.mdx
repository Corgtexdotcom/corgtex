...
- Inline `fontSize`, `padding`, `width`, `minWidth`, or `alignSelf` overrides on buttons, filters, checkbox controls, or table actions are review concerns unless they map to a named design-system class.
