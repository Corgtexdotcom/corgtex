...
| `FLEET_RELEASE_TARGETS_JSON` | Variable | Private configured inventory for managed customer targets. Required for managed-customer selection. |
...
Each target variable may contain one target object or an array of target objects. Keep target inventory operational and sanitized: IDs, URLs, provider IDs, and release metadata are acceptable only inside the private GitHub environment inventory; raw customer content, credentials, private uploads, and bearer tokens are not.
...
Managed-customer targets additionally require `inventoryKey`, `deploymentId`, and a canonical HTTPS origin that exactly matches the Ops control-plane production row. The runner validates the raw managed inventory before dedupe or external reads, then requires exact agreement with control-plane deployment ID, canonical origin, provider resources, environment, deployment kind, lifecycle, and runtime health provider. Managed public output uses only an opaque SHA-256 inventory reference; raw customer labels, origins, deployment IDs, Railway project/environment/service IDs, and Railway source URLs stay out of plans, blockers, GitHub outputs, summaries, advisories, and uploaded artifacts.
...
Only `selfserve` Azure targets are mutable in PR1. Azure managed-customer, core, backup, and Ops targets are shown with blockers and remain non-mutable until the generic Azure executor lands in PR3; they never inherit selfserve resource defaults and do not emit managed-Azure observation targets.
...
6. Confirm the dry-run dispatches `fleet-release-preflight.yml` and prints each target's workload, provider, ring, criticality, deprecations, and blockers before any provider mutation. Managed-customer rows must show opaque inventory references instead of resource identifiers. Ops must be in the final ring.
