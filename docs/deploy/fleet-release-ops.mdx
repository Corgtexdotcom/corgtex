...
| `GHCR_IMPORT_TOKEN` | Secret, required for Ops and backup app | Durable GHCR package-read token for private-image pulls after the workflow ends. Ops and backup-app releases reject the short-lived `GITHUB_TOKEN` fallback. Other Railway targets and Azure imports retain that fallback. |
...
| `GHCR_IMPORT_USERNAME` | Variable, required for Ops and backup app | Identity that owns the durable package-read token. Other targets may fall back to the GitHub Actions actor. |
