...
9. For a real promotion, use a specific support reason and monitor the GitHub Actions run until each target proves matching `gitSha`, `imageTag`, `database=up`, `schema=ready`, passing customer-read probes, and `supportConnectorReadiness.status=ready`. A non-dry-run promotion with `targets=all` requires explicit `--confirm-all` or `confirm_all=true`.
...
Fleet release is for promoting a new product release. Do not use `targets=all` or `railway-customers` to repair one customer's provider variables or worker-only readiness. For configuration repair, call the Ops control-plane `redeploy_customer_services` tool with explicit deployment IDs, selected services, a reason, and optional expected release metadata. The selected redeploy path restarts the current Railway service instances without changing image tags or release variables.
