...
Managed-customer inventory is a private assertion, not public documentation. Each managed entry must carry a generic `inventoryKey`, exact `deploymentId`, canonical HTTPS origin, declared provider, and provider resource assertion. The runner validates the raw managed inventory before dedupe or external reads, then reconciles only selected entries against exactly one production control-plane row and the live `/api/health` `release.provider` before writing target snapshots, selecting provider credentials, emitting observation targets, or dispatching provider commands.

Control-plane classification is authoritative. `REMOTE_MANAGED` rows from supported providers and Railway `HOSTED_DEDICATED` rows classify as `managed-customers`; Azure `HOSTED_DEDICATED` rows classify as `selfserve`; `INTERNAL` rows classify as `backup-app`; other or contradictory rows cannot satisfy managed reconciliation. Every matched managed row must classify as managed, and active managed control-plane rows that are absent from the selected private inventory fail closed.

Managed-customer blockers are intentionally generic. A missing inventory entry, duplicate key/origin/deployment identity/reference, ambiguous or missing control-plane row, lifecycle drift, resource mismatch, unavailable/redirected/malformed health response, or provider disagreement blocks the whole managed selection with codes such as `managed_inventory_missing`, `control_plane_row_ambiguous`, `control_plane_resource_mismatch`, `health_provider_missing`, and `provider_inventory_conflict`. Outputs use exact private inventory keys only inside the runner and full SHA-256 generic references publicly; do not place customer names, domains, deployment IDs, provider resource names, or private mappings in docs, issues, PR bodies, or logs.
