...
<Steps>
  ...
  <Step title="Start the stack">
    ...
    Keep `CORGTEX_RELEASE_GIT_SHA` exported for both the Compose build arguments and the web runtime environment. If you check out a different revision before rebuilding or restarting the stack, export and validate the variable again so the image build, deployment ID, and `/api/health` release metadata all report the same immutable commit SHA.
  </Step>
