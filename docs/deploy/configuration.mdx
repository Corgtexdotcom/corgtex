...
| `CRM_INQUIRY_WORKSPACE_SLUG` | Optional explicit workspace target for the public CRM inquiry route. Corporate Rebels browser submissions use this value instead of the deployment-wide workspace slug. |
| `CRM_INQUIRY_ACKNOWLEDGEMENT_CC_EMAIL` | Comma-separated colleague addresses copied on CRM inquiry acknowledgement emails. The durable email job fails visibly until this is configured; CRM capture remains committed. |
...
| Variable | Description |
|---|---|
| `SHARED_STATE_BACKEND` | `redis` (default) or `postgres`. Set the same value on every web and worker process. PostgreSQL requires the shared-state migration, `DATABASE_URL`, and a valid `ENCRYPTION_KEY`. |
| `REDIS_URL` | Shared Redis connection for rate limits, retrieval cache, and pending transcript uploads when the Redis backend is selected. |
| `REDIS_KEY_PREFIX` | Deployment-specific namespace for either shared backend; keep it identical across web and worker. The historical variable name is retained for compatibility. |
...
Apply the additive migration before selecting `SHARED_STATE_BACKEND=postgres`. Keep existing deployments on their current backend until a coordinated transition. The PostgreSQL path uses shared transactional rate limits and version counters, encrypted cache payloads, and workspace-bound encrypted pending uploads. Database failure denies security-sensitive requests; it never silently switches to process memory or Redis. PostgreSQL cache invalidation commits with PostgreSQL knowledge-source updates.
...
| Value | Behavior |
| --- | --- |
| `continuous` | Polls events and jobs, expires approvals, and runs daily, periodic and drip scheduling. |
| `queue-only` | Polls events and jobs and exposes worker health/readiness. Does not schedule future work. |
| `scheduler-once` | Runs approval expiry and daily, periodic and drip scheduling once, then closes connections and exits. Does not consume queues or open a health listener. |
...
For a single provider workspace, configure `RECALL_API_KEY`, `RECALL_WEBHOOK_SECRET`,
