...
describe("validateFinanceReportEvidenceSourcesV1", () => {
  ...
  it("returns deterministic source-only facts and validates the closed input shape", () => {
    ...
    expect(code(validate("PDF", extracted, [{ ...claims[0], source: { ...claims[0]!.source, extra: true } }]))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, [Object.defineProperty({ ...claims[0] }, "extra", { value: true })]))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, [{ ...claims[0], [Symbol("extra")]: true }]))).toBe("INVALID_INPUT");
