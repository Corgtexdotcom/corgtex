...
describe("validateFinanceReportEvidenceSourcesV1", () => {
  ...
  it("returns deterministic source-only facts and validates the closed input shape", () => {
    ...
    expect(code(validateFinanceReportEvidenceSourcesV1({ format: "PDF", extractedEvidence: extracted,
      claims, extra: true }))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, new Array(1)))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, Object.assign(new Array(1), { foo: claims[0] })))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, [Object.defineProperty({ id: "getter", source: claims[0]!.source }, "role", { enumerable: true, get: () => "TEXT" })]))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, Object.setPrototypeOf([{}], { map: () => claims })))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, Object.defineProperty(new Array(1), "0", { enumerable: true, get: () => claims[0] })))).toBe("INVALID_INPUT");
    expect(code(validate("PDF", extracted, [{ ...claims[0], source: { ...claims[0]!.source, extra: true } }]))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, [Object.defineProperty({ ...claims[0] }, "extra", { value: true })]))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, [{ ...claims[0], [Symbol("extra")]: true }]))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, [{ ...claims[0], source: Object.create({ start: 0 }, Object.getOwnPropertyDescriptors(claims[0]!.source)) }]))).toBe("INVALID_INPUT"); const proxied = new Proxy(claims[0]!, { get: (target, key) => key === "role" ? "NOT_A_ROLE" : Reflect.get(target, key) }); expect(code(validate("PDF", extracted, [proxied]))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, new Proxy(claims, {})))).toBe("INVALID_INPUT"); let descriptorReads = 0; expect(code(validate("PDF", extracted, [new Proxy(claims[0]!, { getOwnPropertyDescriptor: (target, key) => key === "role" ? { value: ++descriptorReads % 2 ? "TEXT" : "ISO_CODE", enumerable: true, configurable: true, writable: true } : Reflect.getOwnPropertyDescriptor(target, key) })]))).toBe("INVALID_INPUT");
