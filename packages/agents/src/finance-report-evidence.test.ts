...
describe("validateFinanceReportEvidenceSourcesV1", () => {
  ...
  it("returns deterministic source-only facts and validates the closed input shape", () => {
    ...
    expect(code(validateFinanceReportEvidenceSourcesV1({ format: "PDF", extractedEvidence: extracted,
      claims, extra: true }))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, new Array(1)))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, Object.assign(new Array(1), { foo: claims[0] })))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, [Object.defineProperty({ id: "getter", source: claims[0]!.source }, "role", { enumerable: true, get: () => "TEXT" })]))).toBe("INVALID_INPUT"); expect(code(validate("PDF", extracted, Object.setPrototypeOf([{}], { map: () => claims })))).toBe("INVALID_INPUT");
