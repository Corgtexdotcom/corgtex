...
const params = { workspaceId: "workspace-synthetic", agentRunId: "run-synthetic",
  ...
  mimeType: "text/csv", extractedEvidence: "{\"sheet\":\"June\",\"row\":2,\"column\":3,\"value\":\"Consulting revenue | 1250.00\"}" };
const withCandidate = (overrides: Partial<FinanceReportImportProposalV1["candidates"][number]>) => ({
  ...proposal(), candidates: [{ ...proposal().candidates[0], ...overrides }],
});
describe("Finance report import proposal contract", () => {
  ...
  it.each([
    ...
    ["reversed candidate period", () => withCandidate({ periodStart: "2026-07-01" })],
    ["candidate outside report period", () => withCandidate({ periodStart: "2026-05-01" })],
    ["fractional cents", () => withCandidate({ amountCents: 12.5 })],
    ["overflow cents", () => withCandidate({ amountCents: 2_147_483_648 })],
    ["empty mapped hierarchy", () => withCandidate({ proposedAccountPath: [] })],
    ["missing location", () => withCandidate({ sourceLocation: { page: null, sheet: null, row: null, column: null, evidence: "Unlocated" } })],
...
describe("interpretFinanceReport", () => {
  it("uses structured extraction and treats approved profiles only as hints", async () => {
    ...
    }] })).resolves.toMatchObject({ contractVersion: 1, candidates: [{ amountCents: 125_000, reviewStatus: "VERIFIED" }] });
  ...
  it.each([
    ...
    expect(result.candidates[0]).toMatchObject({ reviewStatus: "WARNING", exceptionCodes: expect.arrayContaining([exceptionCode]), reviewReasons: [expect.any(String)] });
  });
  ...
  it("points report-window retries at the offending start field", async () => {
    const outside = proposal(); outside.candidates[0].periodStart = "2026-05-01";
  ...
  it("forces a model-supplied review reason out of verified state", async () => {
    const output = proposal(); output.candidates[0].reviewReasons = ["The source label needs human review."];
    const model = gateway([output]);
    await expect(interpretFinanceReport({ ...params, gateway: model.model })).resolves.toMatchObject({ candidates: [{ reviewStatus: "WARNING" }] });
  });
  ...
  it("sanitizes terminal invalid output while preserving transient retries", async () => {
    ...
    expect(caught).toMatchObject({ code: "FINANCE_REPORT_IMPORT_AGENT_INVALID_OUTPUT", message: "The financial report could not be interpreted safely. Please retry." });
