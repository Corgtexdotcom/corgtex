...
function isDeterministicNegativeCategory(text: string, concreteNextStep = "", includeCompletion = true): boolean {
  ...
  const categoryText = /\brun (?:the )?routing test\b/.test(normalizeText(concreteNextStep)) ? splitAssignmentClauses(text).map(clause => /(?:^|\s)(?:[\p{L}\p{N}_-]+\s+){1,3}(?:needs?\s+to|should|must|will)\s+run\s+(?:the\s+)?routing\s*test\b/u.test(clause) ? clause.replace(/\brouting\s*test|test\s*routing\b/gi, "") : clause).join(" ") : text;
