...
describe("Slack context jobs", () => {
  ...
  it.each([
    ...
    { label: "timestamp scaffold does not ground a deliverable", output: { resolutionState: "open", workDisposition: "action", ownerEvidence: "Jan", concreteNextStep: "2026 04", confidence: 0.99, negativeCategory: false, couldNot: [], title: "Invalid timestamp task" }, text: "Jan should send the report by tomorrow." },
    ...
    { label: "anonymous contracted first-person reply", output: { resolutionState: "open", workDisposition: "action", ownerEvidence: "I", concreteNextStep: "send the report", confidence: 0.99, negativeCategory: false, couldNot: [], title: "Send report", reply: "I'll send the report tomorrow." }, replyAuthor: null, text: "Can anyone help?" },
    ...
    { label: "unknown contracted first-person reply", output: { resolutionState: "open", workDisposition: "action", ownerEvidence: "I", concreteNextStep: "send the report", confidence: 0.99, negativeCategory: false, couldNot: [], title: "Send report", reply: "I'll send the report tomorrow." }, replyAuthor: "U404", text: "Can anyone help?" },
    ...
    { label: "multiline punctuated negated task", output: { resolutionState: "open", workDisposition: "action", ownerEvidence: "Jan", concreteNextStep: "send the report", confidence: 0.99, negativeCategory: false, couldNot: [], title: "Send report" }, text: "Jan should not,\nunder any circumstances,\nsend the report; can someone confirm?" },
    ...
    { label: "demonstrative-object no-create", output: { resolutionState: "open", workDisposition: "action", concreteNextStep: "update the guide", ownerEvidence: "", explicitActionRequest: true, confidence: 0.95, title: "Update guide", negativeCategory: false, couldNot: [] }, text: "Corgtex, don't make this an action item to update the guide.", id: "msg-demo-neg" },
  ...
  it.each([
    ...
    { label: "trusted reply author grounds contracted first-person assignment", text: "Can anyone help?", step: "send the report", owner: "I", title: "Send report", bodyMd: "Can anyone help?", replies: ["I'll send the report tomorrow."], replyAuthors: ["U2"] },
    ...
    { label: "trusted owner-intervening explicit create", text: "Corgtex, create a task for Jan to send the report.", step: "send the report", owner: "Jan", title: "Send report", bodyMd: "Corgtex, create a task for Jan to send the report.", explicitActionRequest: true },
    { label: "later assignment supersedes selected-task negation", text: "Jan should not send the report.", step: "send the report", owner: "Jan", title: "Send report", bodyMd: "Jan should not send the report.", replies: ["Actually, Jan should send the report tomorrow."] },
  ...
  ])("creates exactly one Action for $label and no duplicate on later scan", async ({ text, step, owner, title, bodyMd, reply, replies, replyAuthors, explicitActionRequest }) => {
    const source = candidate({ text, messageTs: new Date("2026-04-28T15:30:00.000Z") });
    setupPendingNudge(source);
    ...
    extractMock.mockResolvedValueOnce({
      output: {
        ...
        confidence: 0.95, title, bodyMd, negativeCategory: false, couldNot: [], explicitActionRequest,
      },
    });
    const { runSlackProactiveScan } = await import("./slack-context");
    await expect(runSlackProactiveScan({
    ...
    expect(createWorkItemMock).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ title: step, bodyMd: reply || replies ? [text, ...(replies || [reply])].join("\n") : text, sourceMessageId: "message-1" }));
    setupPendingNudge(source);
