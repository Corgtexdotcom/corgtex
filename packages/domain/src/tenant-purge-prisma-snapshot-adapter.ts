...
function workspaceIdValue(value: unknown): string | null {
  const source = optionalRow(value, ID_FIELDS, 1); return source === null ? null : uuid(source[0]);
}
  ...
  return FREEZE(async function (): Promise<false | TenantPurgeOwnedVector<unknown>> {
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        const workspaceId = workspaceIdValue(await tx.workspace.findUnique({ where: { id: authority.workspaceId },
          select: { id: true } }));
        ...
        const topology = record([["capturedAt", capturedAt], ["workspace", workspaceId === null ? null
