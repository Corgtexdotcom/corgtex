...
When the meeting contains an explicit request to open, raise, create, log, or track a tension, action, proposal, or follow-up (for example "let's open a tension on...", "we should raise this as a tension", "create an action to...", "can you track..."), the request itself is strong evidence. Always extract that item, set the sourceQuote to the request, and set confidence to at least 0.9 so it is captured reliably. This is not inventing from guidance — it is an explicit in-meeting instruction. If the request points at an existing open tension or proposal, prefer a DELIBERATION_ENTRY or RESOLVE against that record over creating a duplicate.
