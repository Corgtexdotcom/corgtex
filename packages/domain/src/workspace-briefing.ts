...
function replaceRawSourceReferences(value: string, sourceRefs: WorkspaceBriefingSourceRef[] = []) {
  ...
  for (const ref of sourceRefs) {
    ...
    readable = readable.replace(
      ...
      (match, offset: number) => (
        isInsideMarkdownLinkDestination(readable, offset) ? match : ref.label
      ),
    );
  }
  RAW_UUID_PATTERN.lastIndex = 0;
  return readable
    .replace(RAW_UUID_PATTERN, (match, offset: number) => (
      isInsideMarkdownLinkDestination(readable, offset) ? match : ""
    ))
    .replace(/[ \t]+([,.;:])/g, "$1")
...
export function normalizeWorkspaceBriefingPayload(input: unknown): NormalizedWorkspaceBriefing {
  ...
  const recordMd = (key: string, fallback: string | null) => {
    ...
    const cleaned = cleanNarrativeMarkdown(value.trim(), recordSourceRefs);
