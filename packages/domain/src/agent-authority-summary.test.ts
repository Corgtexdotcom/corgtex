...
describe("buildAgentAuthoritySummaries", () => {
  it("derives full and default authority summaries from existing inputs", () => {
    const summaries = buildAgentAuthoritySummaries({
      identities: [{
        id: "agent-1", agentKey: "meeting-summary", displayName: "Meeting Summary",
        memberType: "INTERNAL", isActive: true, linkedCredentialId: "cred-1",
        maxSpendPerRunCents: 250, maxRunsPerDay: 8, maxRunsPerHour: 3,
        circleAssignments: [{ circle: { id: "circle-1", name: "Operations" }, role: { id: "role-1", name: "Meeting Steward" } }],
      }],
      configs: [
        {
          agentKey: "meeting-summary", label: "Meeting Summary", category: "knowledge",
          canDisable: false, costTier: "medium", defaultModelTier: "quality",
          enabled: true, modelOverride: "openrouter/custom",
      ...
      credentials: [{
        id: "cred-1", label: "Claude Desktop",
        scopes: ["meetings:read", "tools:credentials:read", "actions:write"],
        isActive: true, lastUsedAt: "2026-06-01T12:00:00.000Z",
        monthlyBudgetCents: 1500, dailyCallLimit: 25,
      }],
      recentRuns: [
        { id: "run-older", agentKey: "meeting-summary", status: "COMPLETED", goal: "Summarize sync", approvalRequired: true, createdAt: "2026-06-01T08:00:00.000Z" },
    ...
    expect(fallback).toMatchObject({
      identityId: null, displayName: "Proposal Drafting", status: "DISABLED", memberType: "INTERNAL",
      roleAssignments: [],
