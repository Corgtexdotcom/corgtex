...
const prismaMock = vi.hoisted(() => ({ $transaction: vi.fn(), $executeRaw: vi.fn(), financeImportBatch: { findUnique: vi.fn(), updateMany: vi.fn() },
  financeReport: { findMany: vi.fn() }, financeImportCandidate: { createMany: vi.fn() }, event: { create: vi.fn() }, auditLog: { create: vi.fn() } }));
...
describe("Finance import proposal persistence", () => {
  ...
  it("atomically stores blocked pre-reconciliation candidates and sanitized state", async () => {
    ...
    await expect(persistFinanceReportImportProposal({ workspaceId: "ws-1", batchId: "batch-1", workflowJobId: "job-1", agentRunId: "run-1",
      ...
      .resolves.toEqual({ batchId: "batch-1", version: 5, stage: "RECONCILING", candidateCount: 1 });
    expect(prismaMock.financeImportCandidate.createMany.mock.calls[0][0].data[0]).toMatchObject({ action: "CONFLICT", reviewState: "BLOCKED",
      semanticKey: null, amountCents: 10_000, periodStart: new Date("2026-01-01T00:00:00.000Z") });
    expect(prismaMock.financeImportBatch.updateMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ version: 4, agentRunId: "run-1" }),
      data: expect.objectContaining({ stage: "RECONCILING", resolvedCurrency: "EUR", interpretationJson: interpretation }) }));
  });
  it("creates no candidates for blockers and records terminal or policy failures safely", async () => {
    ...
    await expect(persistFinanceReportImportProposal({ workspaceId: "ws-1", batchId: "batch-1", workflowJobId: "job-1", agentRunId: "run-1",
      ...
      candidates: [], warningCount: 0, blockerCount: 1, blocker: { code: "CURRENCY_UNRESOLVED", message: "Choose currency." } }))
