/**
 * Represents the complete operational state and evidence for a provider cutover.
 */
...
/**
 * Provides the required environment bounds for a safe operational assessment.
 */
export type CutoverAssessmentContext = {
  assessedAt: Date;
...
/**
 * A machine-safe boolean summary of the allowed operational paths.
 */
export type CutoverReadinessAssessment = {
...
/**
 * Strongly typed error codes representing operational invariants that are not met.
 */
export type CutoverBlockerCode =
  | "INVALID_IDENTITY"
  | "INVALID_CONTEXT"
  | "MALFORMED_RECORD_DATE"
  | "FUTURE_HORIZON"
  | "SOURCE_RUNTIME_DOWN"
  | "SOURCE_RUNTIME_UNOBSERVED"
  ...
  | "MISSING_OBSERVATION"
  | "OBSERVATION_BEFORE_DESTINATION_WRITES"
  | "WAIVER_INCOMPLETE"
  | "WAIVER_FUTURE"
  | "DEADLINE_BEFORE_SNAPSHOT"
  | "RETENTION_NOT_REACHED"
  | "CONTRADICTORY_DELETION"
  | "MISSING_DELETION_EVIDENCE"
  | "FUTURE_EVIDENCE";

/**
 * A normalized, redacted summary of the provider cutover state safe for external logs.
 */
export type SanitizedCutoverSummary = {
  ...
  observationCompletedAt: Date | null;
  archiveRetentionDeadline: Date | null;
  retentionWaiverPresent: boolean;
  sourceDeletedAt: Date | null;
  blockerCodes: CutoverBlockerCode[];
};
...
function fail(s: string | null, sp: string | null, dp: string | null, b: Set<CutoverBlockerCode>) {
  return {
    ...
    summary: {
      ...
      archiveRetentionDeadline: null,
      retentionWaiverPresent: false,
...
/**
 * Assesses the readiness of a provider cutover in a deterministic, pure, and safe way.
 * 
 * @param r The full database record and untrusted evidence.
 * @param c The assessment bounds and current observation time.
 * @returns A safe boolean readiness tuple and a sanitized summary.
 */
export function assessCutoverReadiness(
  r: ProviderCutoverRecord,
  c: CutoverAssessmentContext
): { readiness: CutoverReadinessAssessment; summary: SanitizedCutoverSummary } {
  ...
  if (!okD(c.assessedAt) || !okD(c.requiredSourceFreshThroughAt) || !okD(c.requiredSourceRuntimeObservedAt)) {
    b.add("INVALID_CONTEXT");
    return fail(null, null, null, b);
  }
  ...
  if (!ss || !sp || !dp || sp === dp) {
    b.add("INVALID_IDENTITY");
    return fail(ss, sp, dp, b);
  }

  const isFuture = (d: Date | null) => d !== null && d.getTime() > c.assessedAt.getTime();
  ...
  if (["SHADOW", "CUTOVER", "OBSERVING"].includes(ss)) {
    ...
    if (isRfc3339(ev.sourceRuntimeObservedAt)) {
      ...
      if (okD(obs) && obs.getTime() >= c.requiredSourceRuntimeObservedAt.getTime() && obs.getTime() <= c.assessedAt.getTime()) {
        ot = true;
      }
    }
    let fv = false;
    if (r.sourceDataFreshThroughAt) {
      if (r.sourceDataFreshThroughAt.getTime() > c.assessedAt.getTime()) {
        b.add("SOURCE_DATA_FUTURE");
      } else if (r.sourceDataFreshThroughAt.getTime() >= c.requiredSourceFreshThroughAt.getTime()) {
        fv = true;
      } else {
    ...
    if (["CUTOVER", "OBSERVING"].includes(ss)) {
      if (!r.destinationWriteStartedAt || c.requiredSourceFreshThroughAt.getTime() < r.destinationWriteStartedAt.getTime()) {
        hz = false;
        b.add("HORIZON_BEFORE_DESTINATION_WRITES");
      }
    }
  ...
  const hObs = r.observationCompletedAt !== null;
  const hDel = r.sourceDeletedAt !== null;

  if (hStop && isFuture(r.sourceWriteStoppedAt)) b.add("FUTURE_EVIDENCE");
  if (hSnap && isFuture(r.finalSnapshotAt)) b.add("FUTURE_EVIDENCE");
  if (hRest && isFuture(r.archiveRestoreTestedAt)) b.add("FUTURE_EVIDENCE");
  if (hObs && isFuture(r.observationCompletedAt)) b.add("FUTURE_EVIDENCE");
  if (hDel && isFuture(r.sourceDeletedAt)) b.add("FUTURE_EVIDENCE");

  if (!hStop) b.add("MISSING_WRITE_STOP");
  if (!hSnap) b.add("MISSING_SNAPSHOT");
  if (!hChk) {
    if (r.finalSnapshotChecksum) b.add("MALFORMED_CHECKSUM");
    else b.add("MISSING_CHECKSUM");
  }
  if (!hRest) b.add("MISSING_RESTORE");

  let aOrd = true;
  if (hStop && hSnap && r.finalSnapshotAt!.getTime() < r.sourceWriteStoppedAt!.getTime()) {
    b.add("SNAPSHOT_BEFORE_STOP");
    aOrd = false;
  }
  if (hSnap && hRest && r.archiveRestoreTestedAt!.getTime() < r.finalSnapshotAt!.getTime()) {
    b.add("RESTORE_BEFORE_SNAPSHOT");
    aOrd = false;
  }
  if (hStop && hSnap && hChk && hRest && aOrd && !isFuture(r.sourceWriteStoppedAt) && !isFuture(r.finalSnapshotAt) && !isFuture(r.archiveRestoreTestedAt)) aa = true;
  ...
  if (r.retentionWaiverApprovedAt || r.retentionWaiverApprovedBy || r.retentionWaiverReason) {
    ...
    if (!isComp) {
      b.add("WAIVER_INCOMPLETE");
      wi = true;
    } else if (r.retentionWaiverApprovedAt!.getTime() > c.assessedAt.getTime()) {
      b.add("WAIVER_FUTURE");
      wi = true;
    } else {
      wp = true;
  ...
  if (ss === "DELETED") {
    if (!hDel) b.add("MISSING_DELETION_EVIDENCE");
  } else if (hDel) {
    b.add("CONTRADICTORY_DELETION");
  }

  if (hDel) {
    if (hObs && r.sourceDeletedAt!.getTime() < r.observationCompletedAt!.getTime()) {
      b.add("CONTRADICTORY_DELETION");
    }
    if (hRest && r.sourceDeletedAt!.getTime() < r.archiveRestoreTestedAt!.getTime()) {
      b.add("CONTRADICTORY_DELETION");
    }
  }

  if (ss === "DELETE_ELIGIBLE" && !hDel && !isFuture(r.sourceDeletedAt)) {
    let oVal = true;
    if (!hObs) {
      b.add("MISSING_OBSERVATION");
      oVal = false;
    } else if (isFuture(r.observationCompletedAt)) {
      oVal = false;
    } else if (!r.destinationWriteStartedAt || r.observationCompletedAt!.getTime() < r.destinationWriteStartedAt.getTime()) {
      b.add("OBSERVATION_BEFORE_DESTINATION_WRITES");
      oVal = false;
    }
    let dReach = false, dVal = true;
    if (r.archiveRetentionDeadline) {
      if (hSnap && r.archiveRetentionDeadline.getTime() < r.finalSnapshotAt!.getTime()) {
        b.add("DEADLINE_BEFORE_SNAPSHOT");
        dVal = false;
      } else if (r.archiveRetentionDeadline.getTime() <= c.assessedAt.getTime()) {
        dReach = true;
      } else {
        b.add("RETENTION_NOT_REACHED");
      }
    } else if (!wp) {
      b.add("RETENTION_NOT_REACHED");
    }
  ...
  return {
    ...
    summary: {
      ...
      archiveAvailable: aa,
      deleteEligible: de,
      observationCompletedAt: r.observationCompletedAt,
      archiveRetentionDeadline: r.archiveRetentionDeadline,
      retentionWaiverPresent: wp,
      sourceDeletedAt: r.sourceDeletedAt,
      blockerCodes: Array.from(b).sort(),
    },
  };
}
