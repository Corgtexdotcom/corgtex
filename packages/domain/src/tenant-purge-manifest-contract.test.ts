...
describe("tenant purge manifest normalization", () => {
  ...
  it("uses captured security intrinsics after caller traps mutate ambient methods", () => {
    ...
    const originalExec = RegExp.prototype.exec; let validation: unknown;
    try { const hostile = new Proxy(input({ capabilitySha: "bad" }), { getOwnPropertyDescriptor(value, key) { if (key === "target") RegExp.prototype.exec = () => [] as unknown as RegExpExecArray; return Reflect.getOwnPropertyDescriptor(value, key); } }); try { normalizeTenantPurgeManifestValues(hostile); } catch (error) { validation = error; } } finally { RegExp.prototype.exec = originalExec; }
    ...
    const originalSuper = Object.getPrototypeOf(AppError); const poisoned = () => { throw "owned"; }; let authorityError: unknown; let protectedError: unknown;
    try { const denied = new Proxy(input({ privateAuthority: false }), { getOwnPropertyDescriptor(value, key) { if (key === "privateAuthority") Object.setPrototypeOf(AppError, poisoned); return Reflect.getOwnPropertyDescriptor(value, key); } }); try { normalizeTenantPurgeManifestValues(denied); } catch (error) { authorityError = error; } } finally { Object.setPrototypeOf(AppError, originalSuper); }
