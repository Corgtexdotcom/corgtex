...
describe("self-serve ops domain", () => {
  ...
  it("rejects legacy impersonation creation before reading or mutating workspace data", async () => {
    const { createSelfServeSupportSession } = await import("./self-serve-ops");
    await expect(createSelfServeSupportSession(operatorActor, {
      deploymentId: "deployment-1", targetMemberId: "member-target", reason: "Support",
    })).rejects.toMatchObject({ status: 403, code: "NAMED_SUPPORT_ACCOUNT_REQUIRED" });
  ...
  it.each(["support-token", "expired-token", ""])("rejects every legacy support token including %s", async (token) => {
    const { consumeSelfServeSupportSession } = await import("./self-serve-ops");
    await expect(consumeSelfServeSupportSession({ token })).rejects.toMatchObject({
      status: 403, code: "NAMED_SUPPORT_ACCOUNT_REQUIRED",
    });
    expect(prismaMock.selfServeSupportSession.findUnique).not.toHaveBeenCalled();
