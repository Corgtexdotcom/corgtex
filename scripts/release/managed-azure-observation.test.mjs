...
describe("managed Azure observation verifier", () => {
  ...
  it("locks the workflow to manual default-branch exact-target execution", () => {
    ...
    expect(workflow.match(/workflow_dispatch:/g)).toHaveLength(1); expect(workflow.match(/uses: [^\n]+/g)).toEqual(["uses: actions/checkout@v5", "uses: actions/setup-node@v5", "uses: azure/login@v2", "uses: actions/checkout@v5", "uses: actions/setup-node@v5", "uses: azure/login@v2"]); expect(workflow.match(/node-version: 22\.22\.0/g)).toHaveLength(2);
