...
describe("managed Azure observation verifier", () => {
  ...
  it("locks the workflow to manual default-branch exact-target execution", () => {
    const workflow = readFileSync(new URL("../../.github/workflows/managed-azure-release.yml", import.meta.url), "utf8");
    expect(workflow.match(/workflow_dispatch:/g)).toHaveLength(1); expect(workflow.match(/uses: [^\n]+/g)).toEqual(["uses: actions/checkout@v5", "uses: actions/setup-node@v5", "uses: docker/login-action@v3", "uses: azure/login@v2", "uses: actions/checkout@v5", "uses: actions/setup-node@v5", "uses: docker/login-action@v3", "uses: azure/login@v2"]); expect(workflow.match(/node-version: 22\.22\.0/g)).toHaveLength(2);
    ...
    expect(workflow.match(/MANAGED_AZURE_ACR_NAME: \$\{\{ inputs\.acr_name \}\}/g)).toHaveLength(2); expect(workflow.match(/MANAGED_AZURE_ACR_RESOURCE_GROUP: \$\{\{ inputs\.acr_resource_group \}\}/g)).toHaveLength(2);
