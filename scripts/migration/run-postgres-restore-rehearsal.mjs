...
import { buildSchemaRepresentation } from "./postgres-schema-representation.mjs";
import { SCHEMA_RESTRICT_KEY, SchemaTokenError, isSchemaOperator, schemaTokenDigest, tokenizeSchemaDump } from "./postgres-schema-tokens.mjs";
...
export async function runPostgresRestoreRehearsal(options) {
  ...
  try {
    ...
    const sourceBoundChecks = new Map();
    const boundCheckDeadline = Date.now() + 30_000;
    const sourceChecks = [...sourceConstraintManifest.values()].filter((entry) => entry.type === "CHECK");
    if (sourceChecks.length <= 64) for (const entry of sourceChecks) {
      if (Date.now() >= boundCheckDeadline) break;
    ...
    try {
      ...
      if (schemaDifferenceDiagnostic?.classification === "EXECUTABLE_SCHEMA_DIFFERENCE") {
        ...
        if (checkCandidate !== null) {
          const manifestRows = (manifest) => [...manifest.values()].map(({ key, type, semantics }) => ({ key, type, semantics }));
          schemaRepresentation = buildSchemaRepresentation({
            serverVersion: sourceSettings.serverVersionNumber,
            tokens: sourceSchema.tokens,
            manifest: manifestRows(sourceConstraintManifest),
            check: sourceBoundChecks.get(checkCandidate.source.key) ?? null,
          }, {
            serverVersion: destinationSettings.serverVersionNumber,
            tokens: destinationSchema.tokens,
            manifest: manifestRows(destinationConstraintManifest),
            check: await captureBoundCheck(destinationClient, checkCandidate.destination),
          }, sourceEvidence.schema, { algorithm: destinationSchema.algorithm, digest: destinationSchema.digest });
    ...
    const evidence = {
      schemaVersion: schemaRepresentation === null ? "1.0.0" : "2.0.0",
      ...(schemaRepresentation === null ? {} : { schemaRepresentation }),
      domain,
      sourceRef,
      targetRef,
